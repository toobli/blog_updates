...
const { validateApiWaitlist } = require('../lib/validate');
...
module.exports = async function handler(req, res) {
  ...
  const limit = rateLimit.check(req, 'api-waitlist', { max: 5, windowMs: 10 * 60 * 1000 });
  if (!limit.allowed) {
  ...
  const result = validateApiWaitlist(body);
